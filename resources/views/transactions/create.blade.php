                ...
                <div class="p-6 text-gray-900">
                    ...
                    @csrf
                        <div>
                            ...
                            <x-text-input id="label" name="label" type="text" class="block w-full mt-1" required />
                            <x-input-error :messages="$errors->get('label')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="amount" :value="__('Montant de la transaction')" />
                            <x-text-input id="amount" type="number" step="0.01" name="amount" class="block w-full mt-1" required />
                            <x-input-error :messages="$errors->get('amount')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="transaction_date" :value="__('Date de la transaction')" />
                            <x-text-input type="date" name="transaction_date" id="transaction_date" class="block w-full mt-1" required/>
